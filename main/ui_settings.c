...
void ui_settings_open(void)
{
    ...
    add_section(p, L()->sec_basemap, 420);
    add_label(p, L()->lbl_carto, 0, 452);
    /* full width: CARTO keys are long enough that a 360 px box would show
       a handful of characters at a time */
    s_ta_carto = add_textarea(p, 0, 476, 740, cfg->carto_key, false);
    add_hint(p, L()->hint_carto, 0, 522, 740);

    add_section(p, L()->sec_smart, 574);
    add_label(p, L()->lbl_mqtt, 0, 606);
    s_ta_mqtt = add_textarea(p, 0, 630, 360, cfg->mqtt_uri, false);
    add_hint(p, L()->hint_mqtt, 0, 676, 740);
